#!/usr/bin/env python3
"""rtl-card - PreToolUse hook for ``mcp__visualize__show_widget``.

Companion to install-rtl. The installed rule tells the model to write its
Persian/RTL reply as plain Markdown between ``<md>`` and ``</md>`` in a single
widget call. This hook intercepts that call and rewrites its input
(``hookSpecificOutput.updatedInput``): the Markdown becomes HTML wrapped in a
fixed RTL shell (Vazirmatn, per-block direction resolution, LTR-isolated
code/paths/URLs). The shell is injected here, locally, so the fixed template
never costs model tokens and the styling never drifts.

Fail-safe in two modes: widget calls without the ``<md>`` sentinel (or
unparseable stdin) pass through silently, so ordinary visualizations are
unaffected; a sentinel card whose conversion fails is denied with guidance, so
the model immediately re-sends the reply as a hand-written HTML card and the
raw sentinel never reaches the screen.
"""
import html
import json
import re
...
def convert(md):
    """Convert a small, safe subset of Markdown to HTML (stdlib only)."""
    lines = md.replace("\r\n", "\n").split("\n")
    out = []
    i, n = 0, len(lines)
    while i < n:
        ...
        while i < n:
            ...
            if (not nxt or nxt.startswith(("```", ">", "|", "<svg", "<SVG"))
                    or HEADING.match(nxt) or RULER.match(nxt) or ULIST.match(nxt) or OLIST.match(nxt)):
                break
            buf.append(nxt)
            i += 1
