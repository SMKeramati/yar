...
Three manual install commands ship as skills too: [`install-guards`](skills/install-guards/SKILL.md) (`/yar:install-guards`) and [`install-perms`](skills/install-perms/SKILL.md) (`/yar:install-perms`) — see [Guardrails & permissions](#guardrails--permissions) — plus [`install-rtl`](skills/install-rtl/SKILL.md) (`/yar:install-rtl`), which teaches the machine's **global** `~/.claude/CLAUDE.md` to render Persian/RTL chat replies correctly: the whole reply as plain Markdown in one widget call, styled locally into an RTL card by the plugin's `rtl-card` PreToolUse hook at zero template cost (Vazirmatn, per-block direction, atomic LTR isolation for paths/URLs), English-only plain chat text (intros, status notes, closings), a hand-written-card fallback when the hook is inactive, and a structurally BiDi-safe fallback for plain CLI. Once per machine, idempotent (a managed marker block), re-run to receive rule upgrades.
